...
import { defaultPieColors, type PieArcData, type PieContextValue, type PieData, PieProvider } from "./pie-context";
    ...
    <PieProvider value={contextValue}>
      ...
      >
          ...
          <Group left={center} top={center}>
            {scrubSlicePaths && scrubSliceFills
              ? scrubSlicePaths.map((d, index) =>
                  d ? (
                    <path d={d} fill={scrubSliceFills[index]} key={data[index]?.label ?? index} pointerEvents="none" />
                  ) : null,
                )
              : null}
            {svgChildren}
        ...
        {centerChildren.length > 0 && (
          <div
            className="chart-pie-center pointer-events-none flex items-center justify-center"
            style={{ gridArea: "1 / 1" }}
          >
            {centerChildren}
          </div>
