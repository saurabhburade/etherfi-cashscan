...
import { isClipExcludedComponent, isPostOverlayComponent, isUnderlayComponent } from "./chart-child-passthrough";
import { type ChartContextValue, ChartProvider, type LineConfig, type Margin } from "./chart-context";
...
export function ScatterChartInner({
  ...
  const yScale = getPrimaryYScale(yScales, scaleLinear<number>().range([innerHeight, 0]).domain([0, 100]));

  const dateLabels = useMemo(
    () => data.map((d) => (typeof d.xLabel === "string" ? d.xLabel : shortDateFmt.format(xAccessor(d)))),
    [data, xAccessor],
  );
