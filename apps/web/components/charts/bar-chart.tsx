...
function extractBarConfigs(children: ReactNode): LineConfig[] {
  ...
  forEachChartChild(children, (child) => {
    ...
    const props = child.props as (BarProps & { strokeWidth?: number }) | undefined;
    const isBarComponent = componentName === "Bar" || componentName === "BarSquares";
    const isSeriesComponent = isBarComponent || componentName === "Area" || componentName === "PatternArea";

    if (isSeriesComponent && props?.dataKey) {
      ...
      configs.push({
        ...
        stroke: dotColor,
        strokeWidth: isBarComponent ? 0 : (props.strokeWidth ?? 2),
...
const ChartCore = memo(function ChartCore({
  ...
  const barLines = useMemo(() => lines.filter((line) => line.strokeWidth === 0), [lines]);
  ...
  const categoryAccessor = useCallback(
    (d: Record<string, unknown>): string => {
      ...
      if (value instanceof Date) {
        // The formatted label omits the year, so using it as the scale domain
        // collapses dates such as Nov 27 from different years into one band.
  ...
  const dateLabels = useMemo(
    () =>
      data.map((d) => {
        const value = d[xDataKey];
        return value instanceof Date ? shortDateFmt.format(value) : String(value ?? "");
      }),
    [data, xDataKey],
  );
  ...
  const fakeTimeScale = useMemo(() => {
    const dates = data.map(xAccessorDate);
    const start = dates[0]?.getTime() ?? Date.now();
    const end = dates.at(-1)?.getTime() ?? start;
    const scale = (date: Date) => (categoryScale(date.toISOString()) ?? 0) + bandWidth / 2;
    return Object.assign(scale, {
      domain: () => [new Date(start), new Date(end)],
      range: () => [0, innerWidth] as [number, number],
      invert: (x: number) => new Date(start + (x / Math.max(innerWidth, 1)) * (end - start)),
      copy: () => scale,
    });
  }, [bandWidth, categoryScale, data, innerWidth, xAccessorDate]);
