...
const BarYAxisInner = memo(function BarYAxisInner({
  ...
  const { margin, barScale, bandWidth, barXAccessor, data, dateLabels, hoveredBarIndex } = useChart();
  ...
  const labelsToShow = useMemo(() => {
    ...
    const allLabels = data.map((d, i) => {
      ...
      const label = dateLabels[i] ?? categoryValue;
      const bandY = barScale(categoryValue) ?? 0;
  ...
  }, [barScale, bandWidth, barXAccessor, data, dateLabels, margin.top, showAllLabels, maxLabels]);
