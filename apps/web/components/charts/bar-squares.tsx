...
function cascadeColumnTransition(
  ...
  const baseDuration = typeof base.duration === "number" ? base.duration : animationDurationMs / 1000;
  return {
    ...base,
...
function SquareColumn({
  ...
  const gradientDefs = useGradient ? (
    <defs>
      <linearGradient gradientUnits="userSpaceOnUse" id={gradientId} x1={0} x2={0} y1={baselineY} y2={columnTop}>
...
const BarSquaresInner = memo(function BarSquaresInner({
  ...
  const seriesConfig = barLines[seriesIndex];
      ...
      {data.map((d, i) => {
        ...
        return (
          <SquareColumn
            ...
            index={i}
            isFaded={isFaded}
            key={`bar-squares-${dataKey}-${categoryValue}-${i}`}
...
export function BarSquares(props: BarSquaresProps) {
  ...
  return <BarSquaresInner {...props} bandWidth={bandWidth} barScale={barScale} barXAccessor={barXAccessor} />;
}
...
const BarColumnTrackInner = memo(function BarColumnTrackInner({
  ...
  return (
    <g className={`bar-column-track-${uniqueId}`} style={{ transition: "opacity 0.15s ease-in-out" }}>
      {data.map((d, i) => {
        const categoryValue = barXAccessor(d);
        const bandPos = barScale(categoryValue) ?? 0;
        const effectiveGroupGap = seriesCount > 1 ? groupGap : 0;

        return barLines.map((line, seriesIndex) => (
          <TrackColumn
            animate
            bandPos={bandPos}
            d={d}
...
function TrackColumn({
  ...
  return (
    <rect fill={fill} height={trackHeight} opacity={effectiveOpacity} rx={rx} ry={rx} width={squareSize} x={x} y={0} />
  );
}
...
export function BarColumnTrack(props: BarColumnTrackProps) {
  ...
  return <BarColumnTrackInner {...props} bandWidth={bandWidth} barScale={barScale} barXAccessor={barXAccessor} />;
}
