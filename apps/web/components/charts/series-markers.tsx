...
export function SeriesMarkers({
  ...
  const pointsByIndex = useMemo(() => new Map(points.map((point) => [point.index, point])), [points]);
...
interface SeriesMarkersActiveHighlightProps {
  enabled: boolean;
  pointsByIndex: ReadonlyMap<number, PointAt>;
  markerStyle: MarkerStyle;
  activeScale: number;
}
...
function SeriesMarkersActiveHighlight({
  ...
  const activePoint = pointsByIndex.get(tooltipData.index);
  if (!activePoint) {
    return null;
  }
